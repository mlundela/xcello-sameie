...
- **Roles.** Owners and admins may change data; members read everything and upload/delete receipts (`canEdit` in `$lib/roles`). Every mutating command calls `requireAdmin()` instead of `requireOrgId()` unless members are meant to do it. The hook puts the active-org role on `locals.role`; pages hide edit controls with `page.data.canEdit`, which the `(auth)` layout load provides. The server check is the real one; hiding controls is only convenience. The member-management commands (`invite_member`, `update_member_role`, `remove_member`, `cancel_invite`) call `requireAdmin()` too, for Norwegian refusals; better-auth still enforces its own rules on top (only owners change owners). A sameie always keeps at least one owner or admin.
- Pages call the query directly (`const data = get_x()`) and render it inside `{#await data}` with a daisyUI `loading-spinner`.
- Mutations use single-flight updates. The page names the queries to refresh with `command({...}).updates(theQuery)`, and the command (or `form`) must serve them with `await requested(get_x, 5).refreshAll()` from `$app/server`. Since Kit 2.6x `.updates()` alone refreshes nothing and the page silently keeps the old data (#40). A command may also `await get_x().refresh()` a query no caller passes.
...
- `+page.server.ts` / `+server.ts` exist only where remote functions can't reach: redirect-only loads (`/+page.server.ts` → `/dashboard` or `/login`; `transaksjoner/+page.server.ts` → the current calendar year if it is a period, else the latest; the dashboard defaults to the same year) and binary responses (PDF reports under `rapporter/[year]/`).
...
- **Posted vouchers are never deleted** (bokføringsloven: traceability). Corrections are new vouchers linked by `reversesVoucherId`; they are invisible in the UI and net out in every `voucher_line` sum.
- `reverseBankAutoVoucher` — call it *before* re-categorising or unmatching a transaction, then create the new voucher; every mutation path in `banktransaksjoner.remote.ts` does this inside one `db.transaction`. It posts a `CORRECTION` voucher with the original's lines swapped, on the original's date, and clears `bankTransaction.voucherId`.
- `createOpeningVoucher` / `readOpeningState` — opening balances are `source: 'OPENING'` vouchers. `createOpeningVoucher` takes the full desired state: the first call posts the voucher, later calls post an OPENING correction that reverses the old value of each changed part and posts the new one. Read current state first and pass the parts you aren't changing. A year can have several OPENING vouchers, so always aggregate their lines.
- `syncOpeningBalances` (`balances.ts`) keeps every OPEN year's opening balances equal to the previous year's closing balances, oldest year first, posting corrections only when something changed. The dashboard, Rapporter, the balance PDF, `open_next_year` and onboarding call it before using opening figures. Only a sameie's first year has editable opening balances (`set_opening_balance` refuses later years).
...
### Bank CSV import
`banktransaksjoner.remote.ts` auto-detects four Norwegian bank formats (BN Bank, DNB, SpareBank 1, Sparebanken Vest) by sniffing the header's first field — deliberately ASCII-stable because the encoding may be Windows-1252 with garbled `æøå` (see `decodeBuffer`). Real exports live in `csv/` and `spv-*.csv` but are gitignored (names and account numbers); tests use made-up files. Import rules: the CSV's year(s) must each have an **OPEN** `accounting_period` or the import is rejected; duplicates are skipped on `date|description|amountOre`; `matchingRule` patterns are matched case-insensitively (owner rules → `MATCHED` on account 3600, expense rules → `CATEGORIZED`), and matched rows get an auto-voucher immediately.

Transaction status is `UNMATCHED | MATCHED | CATEGORIZED`. Receipts (`attachment`) arrive through the `upload_attachment` form (raw bytes; a command would base64 them twice), at most 10 MB, typed by file signature in `$lib/server/receipt.ts`, and are stored base64 in Postgres; `receiptNotRequired` defaults true for inflows.
